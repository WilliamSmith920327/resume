...
import { setResumePropertyPerUser, addEmployments, addEducations, IncrementDownloads,addSkills, addLanguages, setJsonPb, getResumeById } from '../../../firestore/dbOperations';
// Animation Library
...
class BoardFilling extends Component {
  ...
  async download() {
    ...
    axios.post('https://' + config.backendUrl + "/api/export", { language: this.props.values.language, resumeId: localStorage.getItem("currentResumeId"), resumeName: this.props.currentResumeName },
      {
  ...
  saveToDatabase() {
    ...
    setTimeout(() => {
      ...
      getResumeById(localStorage.getItem("user"), localStorage.getItem("currentResumeId")).then((data) => {
        if (data != null) {
          localStorage.setItem("currentResumeItem", JSON.stringify({ id: localStorage.getItem("currentResumeId"), item: (data), employments: data.employments, educations: data.educations, skills: data.skills, languages: data.languages }))
        }
      })
    }, 1000);
      ...
      <div className="board">
        ...
        <div id="cv" className="cv">
          <div className="cvWrapper">
            ...
            <div id="Resume">
              <Canvas setToFirstPage={this.setToFirstPage} currentResumeName={this.props.currentResumeName} initialisePages={this.initialisePages} currentPage={this.state.currentPage} pages={this.state.page} addPage={this.addPage} previousPage={this.removePage} downloadEnded={this.downloadEnded} triggerDownload={this.state.triggerDownload} values={this.props.values} />
            </div>
       ...
       */}
            <div className="cvAction">
              ...
              <div>
                ...
                <button onClick={() => this.startDownload()} style={{ fontSize: "15px" }} className={this.state.isSaving== true ? "btn-default isSaving" : "btn-default"}>{t("form.download")} </button>
              </div>
            </div>
          </div>
        </div>
