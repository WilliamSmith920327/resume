...
class DashboardMain extends Component {
  ...
  authListener() {
    fire.auth().onAuthStateChanged((user) => {
      if (user) {
        ...
        getFullName(user.uid).then((value) => { value !== undefined && this.setState({ firstname: value.firstname, lastname: value.lastname,membership:value.membership }) });
  ...
  componentDidMount(){
    getWebsiteData().then((data)=>this.setState({metaDataFetched:true,websiteTitle:data.title,websiteDescription:data.description , websiteKeywords:data.keywords}))
  }
  render() {
    const { t } = this.props;
        ...
        <div className="navbar">
          ...
          <div className="dashboardNavigaitionList">
            <ul>
              ...
              <li><a onClick={() => this.handleDashboardClick()} className={this.state.activeNav == "Dashboard" ? "dashboardNavItem dashboardNavItemActive" : "dashboardNavItem "}>{t("dashboard.dashboard")}</a></li>
              {this.state.pages !== null && this.state.pages.map((value, index) => {
                            return <li>  <Link className="dashboardNavItem" to={{ pathname: "/p/" + value.id }}>{value.id}</Link>  </li>

                        })}
              <li><Link to="contact" className="dashboardNavItem">Contact Us</Link> </li>
            ...
            <AnimatePresence>
              {this.state.isDropdownShowed &&
                <motion.div transition={{ duration: 0.3 }} initial={{ translateX: 400 }} animate={{ translateX: 0 }} exit={{ translateX: 400 }} className="dashboardDropdown">
                  <ProfileDropDown user={this.state.user} handleDashboardClick={this.handleDashboardClick} handleSettingsClick={this.settingsClickHandler} logout={this.logout} />
                </motion.div>}
            </AnimatePresence>
          </div>
        </div>
