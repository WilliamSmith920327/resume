      ...
      resumes[index] = <li key={index} className="resumeItem">
        ...
        <div className="resumeItemContent">
          <div className="resumeItemContentWrapper">
            ...
            <span className="occupation">{this.state.resumes[index].item.title}</span>
          </div>
          <div>
          <Link onClick={() => this.setAsCurrentResume(this.state.resumes[index].id, JSON.stringify(this.state.resumes[index]))} className="btn-default btn-goResume"  to={"/?step=3"}> Go To Resume</Link>
            <a onClick={() => this.deleteResume(localStorage.getItem("user"), this.state.resumes[index].id, index)} className="btn-default btn-removeResume">Remove</a>
          </div>
        </div>
      </li>;
    }
