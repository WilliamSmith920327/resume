...
class Billing extends Component {
    ...
    componentDidMount() {
        ...
        getSubscriptionStatus().then((data) => {
            this.setState({ monthly: data.monthlyPrice, quartarly: data.quartarlyPrice, yearly: data.yearlyPrice , onlyPP:data.onlyPP })
        })
    }
    render() {
            ...
            <Elements stripe={this.stripePromise}>
                <div className="custom-page">
                    ...
                    <div className="custom-page__nav">
                        ...
                        <li>    <Link  className="custom-page__navlinks" to={{pathname:"/"}}>Home</Link> </li>
                            {this.state.pages !== null && this.state.pages.map((value, index) => {
                                return <li>  <Link className="custom-page__navlinks" to={{pathname:"/p/" + value.id}}>{value.id}</Link>  </li>
                                                    ...
                                                    <li>    <Link  className="custom-page__navlinks" to={{pathname:"/contact"}}>Contact us</Link> </li>
                    ...
                    <div className="custom-page__content" >
                        <div className="custom-page__Plans">
                            {this.state.step == 0 && <PlansTable monthly={this.state.monthly} quartarly={this.state.quartarly} yearly={this.state.yearly} nextStep={this.nextStep} />}
                            {this.state.step == 1 &&
                                <ElementsConsumer>
                                    {({ stripe, elements }) => (
                                        <Checkout onlyPP={this.state.onlyPP} previousStep={this.previousStep} stripe={stripe} elements={elements} monthly={this.state.monthly} quartarly={this.state.quartarly} yearly={this.state.yearly} selectedPlan={this.state.selectedPlan} />
                                    )}
                                </ElementsConsumer>
                            }
                        </div>
